...
./harl << EOF > .output.txt 2>&1
DEBUG
EOF
cat << EOF > .expected.txt 2>&1
Enter a complaint level (DEBUG, INFO, WARNING, ERROR): [ DEBUG ]
...
EOF
cat << EOF > .expected.txt 2>&1
Enter a complaint level (DEBUG, INFO, WARNING, ERROR): [ INFO ]
...
EOF
cat << EOF > .expected.txt 2>&1
Enter a complaint level (DEBUG, INFO, WARNING, ERROR): [ WARNING ]
...
EOF
cat << EOF > .expected.txt 2>&1
Enter a complaint level (DEBUG, INFO, WARNING, ERROR): [ ERROR ]
...
EOF
cat << EOF > .expected.txt 2>&1
Enter a complaint level (DEBUG, INFO, WARNING, ERROR): [ Probably complaining about insignificant problems ]
EOF
if (diff -q .expected.txt .output.txt >> /dev/null 2>&1); then
    echo -e "\033[1;32mValide\033[0m"
    echo -e "\033[1;32mValide\033[0m" >> debug.txt
...
valgrind --leak-check=full ./harl << EOF > .valgrind.txt 2>&1
DEBUG
EOF
